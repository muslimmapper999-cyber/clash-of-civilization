...
MESSAGE_OWNER      = {}   # message_id → uid (لمنع الضغط من غير صاحب الرسالة)
...
STRAITS = {
    "هرمز": {
        "controller": ["عمان","ايران"],
        "affects":    ["السعودية","الكويت","العراق","قطر","الامارات"],
        "blocked": False, "blocked_by": None
    },
    "باب المندب": {
        "controller": ["اليمن","جيبوتي"],
        "affects":    ["مصر","الاردن","السودان","ارتريا","الصومال","السعودية","الامارات","اليمن"],
        "blocked": False, "blocked_by": None
    },
    "السويس": {
        "controller": ["مصر"],
        "affects":    ["الاردن","فلسطين","لبنان","سوريا","تركيا","قبرص",
                       "جيبوتي","الصومال","اليمن","السعودية","الكويت","العراق","قطر","الامارات","عمان","ايران"],
        "blocked": False, "blocked_by": None
    },
    "البسفور": {
        "controller": ["تركيا"],
        "affects":    ["جورجيا","اذربيجان","ارمينيا"],
        "blocked": False, "blocked_by": None
    },
    "جبل طارق": {
        "controller": ["المغرب"],
        "affects":    ["الجزائر","تونس","ليبيا","مصر","فلسطين","لبنان","سوريا","تركيا","قبرص","موريتانيا"],
        "blocked": False, "blocked_by": None
    },
}
...
RESOURCE_FACILITIES = {
    ...
    # ===== Lv.4 =====
    "منجم_ذهب":   {"name":"⛏️ منجم ذهب",         "base_cost":80000, "amount":12, "emoji":"⛏️", "infra_req":4,
                   "special":"إنتاج ذهب خام — متاح للدول ذات موارد الذهب فقط",
                   "regions": ["السودان","مصر","الاردن","المغرب","موريتانيا","الصومال","ارتريا","جيبوتي","الامارات","تونس","ليبيا","السعودية","لبنان","قبرص","اذربيجان","ارمينيا","جورجيا"]},
...
CROP_SELL_PRICE = {
    ...
    "نفط":500, "غاز":400, "صلب":350, "ذهب":800, "منجم_ذهب":1200,
}
...
def register_msg(msg, uid):
    """يسجل صاحب الرسالة في MESSAGE_OWNER — يُنظَّف تلقائياً بعد 500 رسالة"""
    MESSAGE_OWNER[msg.message_id] = uid
    if len(MESSAGE_OWNER) > 500:
        # امسح أقدم 100 إدخال
        for k in list(MESSAGE_OWNER.keys())[:100]:
            del MESSAGE_OWNER[k]
...
def get_player(d, uid):
    ...
    if p:
        ...
        defaults = {
            ...
            "fleet": {},
            "merged_regions": [],
            "portfolio":      {},  # {res: {"qty": N, "avg_price": N}}
        }
...
def find_by_name(d, name):
    ...
    for uid, p in d["players"].items():
        ...
        if (clean == name_norm or full == name_norm or
            ...
            return uid, p
        # فحص المناطق المدمجة — "هجوم على ليبيا" يشير لصاحب ليبيا المدمجة
        for merged_r in p.get("merged_regions", []):
            if norm(merged_r) == name_norm:
                return uid, p
    return None, None
...
# ==================== جمع الضرائب يدوياً ====================
def calc_estimated_income(p, data):
    """يحسب الدخل التقديري بنفس منطق do_harvest"""
    crops_p   = p.get("crops", {})
    ...
    for mr in p.get("merged_regions", []):
        for crop in REGION_PREFERRED_CROPS.get(mr, []):
            if crop not in preferred:
                preferred.append(crop)
    oil_bonus   = min(0.60, facs.get("ميناء_نفطي", 0) * 0.20)
    solar_bonus = min(0.48, facs.get("طاقة_شمسية", 0) * 0.08)
    semi_bonus  = min(0.90, facs.get("اشباه_موصلات", 0) * 0.30)
    space_bonus = min(0.60, facs.get("برنامج_فضائي", 0) * 0.20)
    ...
    for res, count in facs.items():
        ...
        total += int(earned * (1 + total_bonus))
    for m_region in p.get("merged_regions", []):
        for res in REGION_RESOURCES.get(m_region, []):
            fc = RESOURCE_FACILITIES.get(res, {})
            qty = fc.get("amount", 2)
            price = CROP_SELL_PRICE.get(res, 400)
            if res in REGION_PREFERRED_CROPS.get(m_region, []): qty = int(qty * 1.5)
            total += int(qty * price * (1 + total_bonus))
    num_proj    = sum(facs.values()) + sum(crops_p.values())
    infra       = p.get("infrastructure", 0)
    terr_income = int((p.get("territories", 1) * 500 + 1000 + num_proj * 300 + infra * 1500) * (1 + total_bonus + ev_tax))
    if min_tax > 0: terr_income = int(terr_income * (1 + min_tax))
    if "خزينة_لا_تنضب" in get_perks(p.get("xp", 0)): terr_income = int(terr_income * 1.20)
    total += terr_income
    for col_p in data.get("players", {}).values():
        if col_p.get("colony_of") != p["country_name"]: continue
        col_income = 0
        for crop, count in col_p.get("crops", {}).items():
            fc = FARM_CROPS.get(crop, {})
            amt = fc.get("amount", 10)
            if crop in REGION_PREFERRED_CROPS.get(col_p.get("region",""), []): amt = int(amt * 1.5)
            col_income += amt * count * CROP_SELL_PRICE.get(crop, 20)
        for res, count in col_p.get("facilities", {}).items():
            fc = RESOURCE_FACILITIES.get(res, {})
            col_income += fc.get("amount", 2) * count * CROP_SELL_PRICE.get(res, 400)
        col_income += col_p.get("territories", 1) * 300 + 500
...
async def do_harvest(app, uid, p, data):
    ...
    region       = p.get("region","")
    # المحاصيل المفضلة تشمل المناطق المدمجة
    preferred    = list(REGION_PREFERRED_CROPS.get(region,[]))
    for mr in p.get("merged_regions", []):
        for crop in REGION_PREFERRED_CROPS.get(mr, []):
            if crop not in preferred:
    ...
    # --- دخل الموارد من المناطق المدمجة --- (100% كأنها دولة واحدة)
    merged_regions = p.get("merged_regions", [])
    if merged_regions:
        merged_lines = []
        merged_total = 0
        for m_region in merged_regions:
            m_resources = REGION_RESOURCES.get(m_region, [])
            m_preferred = REGION_PREFERRED_CROPS.get(m_region, [])
            for res in m_resources:
                fc     = RESOURCE_FACILITIES.get(res, {})
                qty    = fc.get("amount", 2)
                price  = CROP_SELL_PRICE.get(res, 400)
                # بونص المحاصيل المفضلة للمنطقة المدمجة
                if res in m_preferred:
                    qty = int(qty * 1.5)
                earned = int(qty * price * (1 + total_income_bonus))
                if earned > 0:
                    merged_total += earned
                    merged_lines.append(f"  {fc.get('emoji','🌍')} [{m_region}] {res} ← {CUR}{earned:,}")
        if merged_total > 0:
            total += merged_total
            lines.append(f"\n🔗 *موارد المناطق المدمجة* (+{CUR}{merged_total:,}):")
...
def _apply_disaster_to_player(data, uid, d):
    ...
    if d["effect"] == "army":
        ...
        if facs:
            VULNERABLE_FACS = {
                "نفط", "غاز", "صلب", "مصنع_اسلحه", "محطة_تحليه",
                "مزرعة_سمكية", "ميناء", "ميناء_نفطي", "مستشفى",
                "مطار", "طاقة_شمسية", "مصنع_الومنيوم", "تحصين"
            }
            flood_names = ["فيضان", "فيضانات"]
            if any(f in d.get("name","") for f in flood_names):
                VULNERABLE_FACS = {"ميناء", "ميناء_نفطي", "مزرعة_سمكية", "محطة_تحليه", "نفط", "غاز"}
            elif "زلزال" in d.get("name",""):
                VULNERABLE_FACS = {"صلب", "مصنع_اسلحه", "نفط", "غاز", "مصنع_الومنيوم", "تحصين"}

            available = {k: v for k, v in facs.items() if k in VULNERABLE_FACS and v > 0}
            if not available:
        ...
        try:
            ...
            for uid, p in data["players"].items():
                ...
                if elapsed >= 480 and elapsed < TAX_COOLDOWN and reminded < last_tax:
                    # الـ cooldown الفعلي مع مراعاة المضايق
                    actual_cooldown = get_tax_cooldown(data, p.get("region",""))
                    remaining = int(actual_cooldown - elapsed)
                    if remaining <= 0: continue  # جاهز فعلاً
    ...
    while True:
        try:
            async with _harvest_lock:
                ...
                # ─── نمو السكان (كل دورة = 5 دقائق) ───
                for puid, pp in data["players"].items():
                    pop    = pp.get("population", 1_000_000)
                    happy  = calc_happiness(pp)
                    food   = calc_food_security(pp)
                    health = calc_health(pp)
                    at_war = bool(pp.get("at_war"))
                    facs   = pp.get("facilities", {})
                    infra  = pp.get("infrastructure", 0)

                    # معدل النمو الطبيعي الأساسي — يزيد مع السكان (compound growth)
                    # كل 10M سكان بيزيد المعدل الأساسي +0.01%
                    pop_millions   = pop / 1_000_000
                    base_rate      = 0.0003 + (pop_millions * 0.00001)  # ينمو مع الكثافة

                    # مضاعف الرفاه
                    if happy >= 70 and food >= 60 and health >= 70:
                        welfare_mult = 2.5       # ازدهار — انفجار سكاني
                    elif happy >= 55 and food >= 45:
                        welfare_mult = 1.8
                    elif happy >= 40 and food >= 30:
                        welfare_mult = 1.0
                    elif happy < 25 or food < 15:
                        welfare_mult = -2.0      # أزمة — وفيات
                    elif happy < 35 or food < 25:
                        welfare_mult = -0.5
                    else:
                        welfare_mult = 0.3

                    growth = base_rate * welfare_mult

                    # مستشفيات — ترفع معدل البقاء
                    hospitals = facs.get("مستشفي", 0)
                    growth += min(0.0008, hospitals * 0.0002)

                    # بنية تحتية — تدعم النمو
                    growth += min(0.0004, infra * 0.00002)

                    # حرب — خسائر بشرية
                    if at_war:
                        growth -= base_rate * 1.5

                    # حد النمو: مش أكثر من +0.30% ومش أقل من -0.25% لكل دورة
                    growth = max(-0.0025, min(0.0030, growth))
...
def _news_classic(data, pvs, stats):
    ...
    if at_war_list:
        ...
        for name, enemies in at_war_list[:3]:
            # جيب عاصمة المهاجم والمدافع
            att_p = next((p for p in pvs if p.get("country_name","").replace(" (محتلة)","").replace(" (مستعمرة)","") == name), None)
            att_cap = att_p.get("capital","") if att_p else ""
            cap_mention = f" (العاصمة: {att_cap})" if att_cap else ""
            news += f"  ⚔️ *{name}*{cap_mention} تحارب {', '.join(enemies[:2])} — _{random.choice(w_cmts)}_\n"
        news += "\n"
    else:
    ...
    if occupied:
        ...
        for occ_name, by_who in occupied[:3]:
            # جيب عاصمة الدولة المحتلة
            occ_p = next((p for p in pvs if p.get("country_name","").replace(" (محتلة)","").replace(" (مستعمرة)","") == occ_name), None)
            occ_cap = occ_p.get("capital","") if occ_p else ""
            cap_txt = f" (عاصمتها {occ_cap} سقطت)" if occ_cap else ""
            news += f"  • *{occ_name}*{cap_txt} تحت سيطرة *{by_who}* — _'ما نعلق'_ 😶\n"
        news += "\n"
    if orgs:
...
async def stock_market_loop(app):
    """تحديث البورصة كل 10 دقائق"""
    await asyncio.sleep(90)
    while True:
        try:
            if not load_data().get("stock_loop_enabled", True):
            ...
            for res, info in data["stock_market"].items():
                ...
                momentum = trend * 0.03  # الاتجاه السابق يؤثر
                change = random.uniform(-0.15, 0.15) + momentum
        ...
        await asyncio.sleep(60 * 10)  # كل 10 دقائق
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ======= فحص إيقاف اللعبة =======
    if data.get("game_paused") and not is_admin(uid):
        await update.message.reply_text(
            "⏸️ *اللعبة موقوفة مؤقتاً*\n"
            "الإدارة أوقفت اللعبة — ارجع لاحقاً! 🕐",
            parse_mode="Markdown")
        return
    ...
    if reply_to and uid in REGISTRATION_STATE:
        ...
        # خطوة 1: اللاعب بعت الاسم — ننتقل لخطوة العاصمة
        if state["step"] == "waiting_name" and reply_to.message_id == state["name_msg_id"]:
            ...
            state["step"] = "waiting_capital"
            cap_msg = await update.message.reply_text(
                f"🏛️ *ممتاز!* دولتك: *{cname}*\n\n"
                f"دلوقتي اكتب اسم *عاصمة* دولتك 🏙️\n"
                f"_(ريبلاي على الرسالة دي بالاسم)_",
                parse_mode="Markdown")
            state["capital_msg_id"] = cap_msg.message_id
            state["started_at"] = time.time()
            return

        # خطوة 2: اللاعب بعت اسم العاصمة — نسجله ونطلب العلم
        if state["step"] == "waiting_capital" and reply_to.message_id == state.get("capital_msg_id"):
            capital = update.message.text.strip() if update.message.text else ""
            if not capital or len(capital) < 2:
                await update.message.reply_text("❌ اسم العاصمة قصير جداً! حاول تاني."); return
            if len(capital) > 40:
                await update.message.reply_text("❌ اسم العاصمة طويل جداً! (حد أقصى 40 حرف)"); return
    ...
    if ntext in ["بناء مزرعه","ابني مزرعه","انشئ مزرعه"]:
        ...
        infra     = p.get("infrastructure", 0)
        merged_regs = p.get("merged_regions", [])
        # المحاصيل المفضلة = منطقتك + كل المناطق المدمجة
        preferred = list(REGION_PREFERRED_CROPS.get(region, []))
        for mr in merged_regs:
            for crop in REGION_PREFERRED_CROPS.get(mr, []):
                if crop not in preferred:
        ...
        if total_farms >= max_farms:
            await update.message.reply_text(
                ...
                f"Lv.{infra+1} ← {get_max_farms(infra+1, region, merged_regs)} مزرعة",
                parse_mode="Markdown"); return
        sent = await update.message.reply_text(
            f"🌾 *اختار المحصول:*\n{limit_txt}\n{pref_txt}\n{sep()}\n{table}\n⭐ = إنتاج أعلى 50%",
            reply_markup=InlineKeyboardMarkup(kbd), parse_mode="Markdown")
    ...
    if ntext in ["احصد دولي","حصاد دولي","احصد كل شي","احصد الكل"]:
        ...
        p = get_player(data, uid)
        if not p: await update.message.reply_text("❌ مش مسجل."); return
        ok, err = check_sovereignty(p, "حصاد")
        if not ok: await update.message.reply_text(err, parse_mode="Markdown"); return
        my_name = p["country_name"]
        now = time.time()
        total_earned = 0
        ...
        # 1. دولتك الأم
        cooldown = get_tax_cooldown(data, p.get("region",""))
        left = cooldown - (now - p.get("last_tax", 0))
        if left <= 0:
            async with _harvest_lock:
                data2 = load_data()
                p2    = get_player(data2, uid)
                await do_harvest(context.application, uid, p2, data2)
                earned = data2["players"][str(uid)]["gold"] - p2["gold"]
                total_earned += max(0, earned)
                save_data(data2)
                data = load_data()
            summary_lines.append(f"🏛️ *{my_name}*: +{CUR}{max(0,earned):,}")
        else:
            m, s = int(left)//60, int(left)%60
            summary_lines.append(f"🏛️ *{my_name}*: ⏳ {m:02d}:{s:02d}")

        # 2. المستعمرات — 60% من الدخل الكامل
        for col_uid, col_p in list(data["players"].items()):
            if col_p.get("colony_of") != my_name: continue
            col_cd   = get_tax_cooldown(data, col_p.get("region",""))
            col_left = col_cd - (now - col_p.get("colony_last_harvest", 0))
            cname = col_p["country_name"].replace(" (مستعمرة)","")
            if col_left <= 0:
                col_full, _, _ = calc_colony_harvest(col_p)
                col_income = int(col_full * 0.60)
                data["players"][str(uid)]["gold"] += col_income
                data["players"][col_uid]["colony_last_harvest"] = now
                total_earned += col_income
                summary_lines.append(f"🏴 *{cname}*: +{CUR}{col_income:,}")
            else:
                m, s = int(col_left)//60, int(col_left)%60
                summary_lines.append(f"🏴 *{cname}*: ⏳ {m:02d}:{s:02d}")

        # 3. الدول المحتلة — 40% من الدخل التقديري
        for occ_uid, occ_p in list(data["players"].items()):
            if occ_p.get("occupied_by") != my_name: continue
            occ_cd   = get_tax_cooldown(data, occ_p.get("region",""))
            occ_last = occ_p.get("occ_last_harvest", 0)
            occ_left = occ_cd - (now - occ_last)
            oname = occ_p["country_name"].replace(" (محتلة)","")
            if occ_left <= 0:
                occ_income = int(calc_estimated_income(occ_p, data) * 0.40)
                data["players"][str(uid)]["gold"] += occ_income
                data["players"][occ_uid]["occ_last_harvest"] = now
                total_earned += occ_income
                summary_lines.append(f"🔴 *{oname}* (محتلة): +{CUR}{occ_income:,}")
            else:
                m, s = int(occ_left)//60, int(occ_left)%60
                summary_lines.append(f"🔴 *{oname}* (محتلة): ⏳ {m:02d}:{s:02d}")

        save_data(data)
        data = load_data()
        new_gold = data["players"][str(uid)]["gold"]
        summary = "\n".join(summary_lines) or "لا شيء جاهز بعد"
        await update.message.reply_text(
            f"{box_title('💰','حصاد دولي')}\n\n"
            f"{summary}\n"
            f"{sep()}\n"
            f"💰 إجمالي مكتسب: *+{CUR}{total_earned:,}*\n"
            f"🏦 رصيدك الكلي: *{CUR}{new_gold:,}*",
            parse_mode="Markdown")
        return
    ...
    if ntext.startswith("استعمر "):
        ...
        await update.message.reply_text(
            ...
            f"🌾 `احصد دولي` — لحصاد كل ممتلكاتك دفعة واحدة\n"
    ...
    if ntext.startswith("اضرب "):
        ...
        if w.get("occupy") and new_army == 0:
            ...
            # نقل المناطق المغزوة (unoccupied_territories)
            for region, info in data.get("unoccupied_territories", {}).items():
                if norm(info.get("occupied_by","")) == norm(clean_name):
                    data["unoccupied_territories"][region]["occupied_by"] = p["country_name"]
            occupied_txt = (
    ...
    if ntext in ["بناء بنيه تحتيه","بنيه تحتيه"]:
        ...
        if infra < 5:
            ...
            cost = 2150000 + (infra - 30) * 300000
        else:
            cost = 8150000 + (infra - 50) * 600000
    ...
    if (ntext.startswith("تغيير اسم دولتي ") or ntext.startswith("تغيير اسم ")) \
        ...
        # تحقق من تكرار الاسم
        for puid, pp in data["players"].items():
            if puid == str(uid): continue
            if norm(pp.get("country_name","").replace(" (محتلة)","").replace(" (مستعمرة)","")) == norm(new_name):
                await update.message.reply_text(f"❌ الاسم *{new_name}* مأخوذ من دولة أخرى!", parse_mode="Markdown"); return
        old_name = p["country_name"].replace(" (محتلة)","").replace(" (مستعمرة)","")
        suffix   = " (محتلة)" if p.get("occupied_by") else (" (مستعمرة)" if p.get("colony_of") else "")
        # حدّث الاسم
        data["players"][str(uid)]["country_name"] = new_name + suffix
    ...
    for action in ["اغلق","افتح"]:
        if ntext.startswith(f"{action} مضيق "):
            ...
            # كل المناطق اللي تحت سيطرة اللاعب
            my_regions = {p["region"]}
            my_name = p["country_name"]
            for _, pp in data["players"].items():
                if pp.get("colony_of") == my_name or pp.get("occupied_by") == my_name:
                    my_regions.add(pp.get("region",""))
            for mr in p.get("merged_regions", []):
                my_regions.add(mr)

            if not my_regions.intersection(set(controllers)):
                await update.message.reply_text(
                    f"❌ دولتك أو ممتلكاتك مش من المتحكمين في مضيق {sname}.\n"
                    f"المتحكمون: {', '.join(controllers)}", parse_mode="Markdown"); return

            # لو في أكثر من متحكم — الأقوى عسكرياً هو المسيطر
            if len(controllers) > 1:
                armies = {}
                for puid2, pp in data["players"].items():
                    pp_region = pp.get("region","")
                    # الدولة الأم
                    if pp_region in controllers:
                        existing = armies.get(pp_region, (0,""))
                        if pp.get("army",0) >= existing[0]:
                            armies[pp_region] = (pp.get("army",0), pp["country_name"])
                    # المستعمرات والمحتلات
                    pp_owner = pp.get("colony_of") or pp.get("occupied_by")
                    if pp_owner and pp_region in controllers:
                        o_uid2, o_p = find_by_name(data, pp_owner)
                        if o_p:
                            existing = armies.get(pp_region, (0,""))
                            if o_p.get("army",0) > existing[0]:
                                armies[pp_region] = (o_p.get("army",0), o_p["country_name"])

                if armies:
                    dominant_region = max(armies, key=lambda r: armies[r][0])
                    dom_owner = armies[dominant_region][1]
                    if dom_owner != my_name:
                        await update.message.reply_text(
                            f"❌ *{dom_owner}* تسيطر على مضيق {sname} بجيش أقوى!\n"
                            f"جيشك: {p['army']:,} | جيشهم: {armies[dominant_region][0]:,}",
                            parse_mode="Markdown"); return
                # لو مفيش لاعبين في المناطق المتحكمة — أي متحكم يقدر يفتح/يغلق
    ...
    if ntext.startswith("غزو "):
        ...
        # تحقق مش محتلة بالفعل — استخدم norm() للمقارنة
        unoccupied = data.setdefault("unoccupied_territories", {})
        matched_occ = next((r for r in unoccupied if norm(r) == norm(target_region)), None)
        if matched_occ:
            occupier = unoccupied[matched_occ].get("occupied_by","")
        ...
        # cooldown الغزو — 2 ساعة بين كل غزو
        GHAZW_COOLDOWN = 3600
        last_ghazw = p.get("last_ghazw", 0)
        ghazw_left = GHAZW_COOLDOWN - (time.time() - last_ghazw)
        if ghazw_left > 0:
            h, m = int(ghazw_left)//3600, (int(ghazw_left)%3600)//60
            await update.message.reply_text(
                f"⏳ *كولدون الغزو!*\n"
                f"لازم تنتظر *{h}س {m}د* قبل الغزو مجدداً.", parse_mode="Markdown"); return
        # شروط
        min_army = 200
        if get_level(p.get("xp", 0))["level"] < 3:
            await update.message.reply_text(
        ...
        data["players"][str(uid)]["last_ghazw"] = time.time()
    ...
    if ntext.startswith("تحرير "):
        ...
        # ===== أولاً: فحص unoccupied_territories (مناطق مغزوة) =====
        unoccupied = data.get("unoccupied_territories", {})
        matched_region = None
        for region, info in unoccupied.items():
            if norm(region) == norm(target_name):
                occ_by = norm((info.get("occupied_by") or "").replace(" (محتلة)","").replace(" (مستعمرة)",""))
                if occ_by == my_clean:
                    matched_region = region
                    break
        if matched_region:
            del data["unoccupied_territories"][matched_region]
            save_data(data)
            await update.message.reply_text(
                f"🕊️ *تم التحرير!*\n{sep()}\n"
                f"تركت منطقة *{matched_region}* حرة\n"
                f"لم تعد تحت سيطرتك.", parse_mode="Markdown")
            return

        # ===== ثانياً: فحص players (دول محتلة أو مستعمرة) =====
        ...
        for tuid, tp in data["players"].items():
            if (norm(tp.get("country_name","").replace(" (محتلة)","").replace(" (مستعمرة)","")) == norm(target_name) or
                ...
                break
        if not found_p:
            await update.message.reply_text(f"❌ مش لاقي دولة أو منطقة اسمها '{target_name}'."); return
    ...
    if ntext.startswith("هجوم علي "):
        ...
        if att > deff:
            ...
            if conquered:
                ...
                # نقل المناطق المغزوة لـ X
                for region, info in data.get("unoccupied_territories", {}).items():
                    if norm(info.get("occupied_by","")) == norm(old_owner_clean):
                        data["unoccupied_territories"][region]["occupied_by"] = p["country_name"]
            ...
            if conquered:
                def_cap_news = data["players"][tuid].get("capital","")
                channel_news = (
                    f"🏴 *خبر عاجل — سقوط عاصمة!*\n{sep()}\n"
                    f"جيوش *{p['country_name']}*"
                    f"{(' من ' + p.get('capital','')) if p.get('capital') else ''}"
                    f" دخلت {'عاصمة *' + def_cap_news + '*' if def_cap_news else 'أراضي'}"
                    f" *{tp['country_name'].replace(' (محتلة)','')}*\n"
                    f"🚩 سقطت تحت الاحتلال الكامل!"
                )
                await send_to_channel(context.bot, data, channel_news)
            try:
                att_capital = p.get("capital","")
                defeat_msg = (
                    f"{box_title('🚨','تنبيه حرب!')}\n"
                    f"*{p['country_name']}*{(' (' + att_capital + ')') if att_capital else ''} هاجمك!\n"
                    f"{sep()}\n"
                    f"💸 خسرت:  {CUR}{loot:,}\n"
                    f"💀 خسائر: {ld:,} جندي\n"
                ...
                if conquered:
                    def_cap = tp.get("capital","")
                    defeat_msg += (
                        f"\n\n🏴 *دولتك محتلة!*\n"
                        f"{'جيوشهم دخلت ' + def_cap + ' 😔' + chr(10) if def_cap else ''}"
                        f"اكتب `ثورة` لمحاولة التحرر"
                    )
    ...
    if ntext in ["بورصه","البورصه","بورصة","البورصة","الاسواق","اسواق","سوق مالي"]:
        if not data.get("market_enabled", True) and not is_admin(uid):
        ...
        for res, info in stock.items():
            ...
            pct_txt = f"+{pct}%" if pct > 0 else f"{pct}%"
            # رسم بياني صغير من التاريخ
            hist = info.get("history", [info["price"]])
            chart = "".join("█" if h >= hist[-1] else "▁" for h in hist[-6:]) if len(hist) > 1 else "─"
            lines.append(f"  {t} *{res}*: {info['price']:,}¥  {pct_txt}  `{chart}`")

        last_upd = data.get("last_stock_update", 0)
        upd_ago  = int(time.time() - last_upd) // 60 if last_upd else 0
        next_upd = max(0, 10 - upd_ago)
        ev   = data.get("world_event")
        ev_ends = data.get("world_event_ends", 0)
        ev_txt = ""
        if ev and time.time() < ev_ends:
            rem = int(ev_ends - time.time())
            ev_txt = f"\n🌍 *حدث عالمي:* {ev['emoji']} {ev['name']}"

        msg = (
            f"{box_title('📊','البورصة العالمية')}\n\n"
            + "\n".join(lines) +
            f"\n{sep()}\n"
            f"🔄 آخر تحديث: منذ {upd_ago}د | القادم: ~{next_upd}د"
            f"{ev_txt}\n\n"
            f"💡 `شراء أسهم [مورد] [عدد]` | `بيع أسهم [مورد] [عدد]`\n"
    ...
    # ======= محفظة الأسهم =======
    if ntext in ["محفظتي","محفظه","محفظة","اسهمي"]:
        p = get_player(data, uid)
        if not p: await update.message.reply_text("❌ مش مسجل."); return
        ...
        if not portfolio:
            await update.message.reply_text(
                f"{box_title('💼','محفظتي')}\n\n"
                f"📭 محفظتك فارغة!\n"
                f"💡 `شراء أسهم [مورد] [عدد]` للبدء",
                parse_mode="Markdown"); return
        total_invested = 0
        total_current  = 0
        lines = []
        for res, holding in portfolio.items():
            qty       = holding.get("qty", 0)
            avg_price = holding.get("avg_price", 0)
            cur_price = stock.get(res, {}).get("price", avg_price)
            invested  = avg_price * qty
            current   = cur_price * qty
            pnl       = current - invested
            pnl_pct   = round(pnl / invested * 100, 1) if invested else 0
            pnl_icon  = "🟢" if pnl >= 0 else "🔴"
            pnl_txt   = f"+{pnl:,}¥ (+{pnl_pct}%)" if pnl >= 0 else f"{pnl:,}¥ ({pnl_pct}%)"
            trend_icon = "📈" if stock.get(res,{}).get("trend",0) > 0 else ("📉" if stock.get(res,{}).get("trend",0) < 0 else "➡️")
            lines.append(
                f"  {trend_icon} *{res}* × {qty:,}\n"
                f"     شراء: {avg_price:,}¥ | حالي: {cur_price:,}¥\n"
                f"     {pnl_icon} {pnl_txt}"
            )
        ...
        msg = (
            ...
            + "\n\n".join(lines) +
            f"\n{sep()}\n"
            f"💰 إجمالي استثمار: {total_invested:,}¥\n"
            f"📊 القيمة الحالية: {total_current:,}¥\n"
            f"{total_icon} الربح/الخسارة: "
            + (f"+{total_pnl:,}¥ (+{total_pnl_pct}%)" if total_pnl >= 0 else f"{total_pnl:,}¥ ({total_pnl_pct}%)")
        )
        await safe_md(update.message, msg)
        return

    # ======= شراء/بيع أسهم =======
    if ntext.startswith("شراء اسهم ") or ntext.startswith("شراء أسهم ") or \
       ntext.startswith("بيع اسهم ")   or ntext.startswith("بيع أسهم ") or \
       ntext.startswith("بيع موارد ")  or ntext.startswith("شراء موارد "):
        p = get_player(data, uid)
        if not p: await update.message.reply_text("❌ مش مسجل."); return
        if not data.get("market_enabled", True) and not is_admin(uid):
            await update.message.reply_text("🚫 *البورصة موقوفة حالياً.*", parse_mode="Markdown"); return
        ok, err = check_sovereignty(p, "تداول")
        if not ok: await update.message.reply_text(err, parse_mode="Markdown"); return
        # rate limit
        ...
        if not matched:
            await update.message.reply_text(
                f"❌ *{res_name}* غير موجود في البورصة.\nالمتاح: {', '.join(stock.keys())}",
                parse_mode="Markdown"); return
        price = stock[matched]["price"]
        portfolio = p.get("portfolio", {})

        if is_sell:
            holding = portfolio.get(matched, {})
            owned_qty = holding.get("qty", 0)
            if owned_qty < qty:
                await update.message.reply_text(
                    f"❌ عندك بس *{owned_qty:,}* سهم من *{matched}*!", parse_mode="Markdown"); return
            avg_price = holding.get("avg_price", price)
            total     = price * qty
            ...
            if new_qty <= 0:
                portfolio.pop(matched, None)
            else:
                portfolio[matched]["qty"] = new_qty
            data["players"][str(uid)]["gold"]             = p["gold"] + total
            data["players"][str(uid)]["portfolio"]        = portfolio
            data["players"][str(uid)]["last_market_trade"] = time.time()
            log_event(data, f"{p['country_name']} باعت {qty} سهم {matched} بـ {CUR}{total:,}", "💹")
            save_data(data)
            await update.message.reply_text(
                f"💹 *بيع ناجح!*\n{sep()}\n"
                f"📦 {qty:,}× {matched} × {price:,}¥\n"
                f"💰 +{total:,}¥\n"
                f"{pnl_icon} ربح/خسارة: {pnl_txt}\n"
                f"🏦 رصيدك: {p['gold']+total:,}¥", parse_mode="Markdown")
        else:
            total = price * qty
            if p["gold"] < total:
                await update.message.reply_text(
                    f"❌ تحتاج {CUR}{total:,}. عندك {CUR}{p['gold']:,}."); return
            # تحديث المحفظة — حساب متوسط سعر الشراء
            holding   = portfolio.get(matched, {"qty": 0, "avg_price": 0})
            old_qty   = holding["qty"]
            old_avg   = holding["avg_price"]
            new_qty   = old_qty + qty
            new_avg   = int((old_avg * old_qty + price * qty) / new_qty)
            portfolio[matched] = {"qty": new_qty, "avg_price": new_avg}
            data["players"][str(uid)]["gold"]             = p["gold"] - total
            data["players"][str(uid)]["portfolio"]        = portfolio
            data["players"][str(uid)]["last_market_trade"] = time.time()
            log_event(data, f"{p['country_name']} اشترت {qty} سهم {matched} بـ {CUR}{total:,}", "🛒")
            save_data(data)
            await update.message.reply_text(
                f"🛒 *شراء ناجح!*\n{sep()}\n"
                f"📦 {qty:,}× {matched} × {price:,}¥\n"
                f"💸 -{total:,}¥\n"
                f"💼 إجمالي {matched}: {new_qty:,} سهم\n"
                f"📊 متوسط سعر الشراء: {new_avg:,}¥\n"
                f"🏦 رصيدك: {p['gold']-total:,}¥", parse_mode="Markdown")
        return
    ...
    if ntext in ["البنك الدولي","بنك","قرض","اخد قرض"]:
        ...
        if loans_txt:
            msg += f"\n📋 *قروضك الحالية:*\n{loans_txt}"
        msg += f"\n{sep()}\n*اختار نوع القرض:*"
        sent = await update.message.reply_text(msg, reply_markup=InlineKeyboardMarkup(rows), parse_mode="Markdown")
        register_msg(sent, uid)
        return
    ...
    if ntext in ["مهرجان شعبي", "مهرجان", "ارضي الشعب", "رضا الشعب"]:
        ...
        sent = await update.message.reply_text(msg, reply_markup=InlineKeyboardMarkup(kbd), parse_mode="Markdown")
        register_msg(sent, uid)
        return
    ...
    if ntext.startswith("معاهده سلام مع ") or ntext.startswith("معاهدة سلام مع "):
        ...
        try:
            ...
            sent_peace = await context.bot.send_message(
                chat_id=int(tuid),
            ...
            register_msg(sent_peace, int(tuid))
        except: pass
        return
